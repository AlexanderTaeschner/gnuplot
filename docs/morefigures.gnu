...
# Example of contour plot with autogenerated numerical labels
# mixed with text labels

set output out . 'figure_contour_labels' . ext
...
EOD
    ...
    set view map
    unset tics
    set margins screen 0.05, screen 0.95, screen 0.05, screen 0.95
    unset key
    g(x,y) = x**2 + y**2 * (1 - x)**3
    set isosamples 99
    set xrange [-1:4]; set yrange [-3:3]
    set cntrparam levels incremental -20,10,20
    set cntrlabel onecolor
    set style textbox opaque
    splot g(x,y) with lines lw 3 nosurface, \
          g(x,y) with labels font "Times,9" boxed, \
          $LABELS using 1:2:(0):3 with labels nocontours
reset
