...
support for older systems as well, including Ultrix, OS/2, and
MS-DOS. However 16-bit platforms are no longer supported.
VMS-specific code was removed from the distribution source in 2026.
...
The current stable version of \gnuplot is 6.0, first released in December 2023.
...
Version 6 introduced a new plot style \textit{with sectors} that makes
pie charts much easier.
...
The same limitation arose when you resized a multiplot displayed in an interactive
...
Once the closing command \textit{unset multiplot} is issued, this data block
can be replayed to redraw the corresponding set of plots that make up the multiplot.
This can be done explicitly using the \textit{remultiplot} command or implicitly
from \textit{replot} commands or resizing events if the immediately preceeding plot
command was part of a multiplot.  It is either a feature or a drawback that the
result of replaying the original commands may produce a slightly different result
if the state of the program (axis ranges, linetypes, fonts, etc) has changed in
the interim.
...
Revised for \gnuplot versions 5 and 6 by Ethan Merritt.
