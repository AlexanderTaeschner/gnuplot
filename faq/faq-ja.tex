...
% Original: Id: faq.tex updated at 2023-12-11
%
...
%%% The current stable version of \gnuplot is 6.0, first released in
%%%  December 2023.
%%% Incremental versions (patchlevel 1, 2, ...) are typically released 
%%% every six months.
%%% The development version is currently \gnuplot 6.1.
\gnuplot の最新の安定版は、2023 年 12 月に最初にリリースされた 6.0 です。
更新バージョン (パッチレベル 1, 2, ...) は、だいたい 6 ヶ月毎に
リリースされます。
現在の開発版は \gnuplot 6.1 です。
...
%%% The source distribution ("gnuplot-6.0.0.tar.gz" or a similar name) is
%%% available from the official distribution site 
%%% \http{sourceforge.net/projects/gnuplot}.
ソース配布物 ("gnuplot-6.0.0.tar.gz" または同様の名前) %
...
%%% Starting with version 5, \Gnuplot supports named blocks of data in 
%%% "here document" format:
\gnuplot のバージョン 5 以降では、「ヒアドキュメント」形式での
...
%%% In version 6 there is also a command {\em set isotropic}.
{\em set size square} か %
{\em set view equal xy} としてみてください。
バージョン 6 では、{\em set isotropic} というコマンドもあります。
...
%%% Version 6 also supports animation in webp format.
バージョン 6 では、webp フォーマットでのアニメーションも
サポートしています。
%% end Japanese
%%% Have a look at
...
%%% Pie charts were difficult in older \gnuplot versions but see
...
%%% Version 6 introduced a new plot style \textit{with sectors} that 
%%% makes pie charts much easier.
円グラフは古い \gnuplot ではちょっと面倒でした。以下を見てください。
\http{gnuplot.sourceforge.net/demo/circles.html} 
または、以下を参照してください。
\http{gnuplot-tricks.blogspot.com/2009/08/pie-charts-entirely-in-gnuplot.html}
バージョン 6 では、新しい描画スタイル \textit{with sectors} を導入
し、もっと簡単に円グラフが描けるようになっています。
...
%%% Plot style \textit{with polygons} can handle polygonal faces
%%% (triangles, rectangles, octagons, ...) in either 2D or 3D plots.
%%% This style was introduced in version 5.4.
描画スタイル \textit{with polygons} は、2 次元描画でも、
3 次元描画でも多角形形状 (三角形、四辺形、八角形、...) を処理できます。
これはバージョン 5.4 で導入されました。
...
%%% \Gnuplot versions through 5.4 retained only enough information to regenerate
%%% the most recent {\em plot} or {\em splot} command.
%%% In order to reproduce the entire multiplot you needed to save the complete 
%%% sequence of commands that generated it into a script file.
%%% Then you could {\em load} that script into \gnuplot as many times as needed
%%% for replotting the drawing to different terminals or output files.
バージョン 5.4 の \gnuplot は、直近の {\em plot} や {\em splot} コ
マンドを再生成するために十分な情報を保持するだけだったので、
multiplot 全体を完全に再現するには、それを生成する完全なコマンド列を
スクリプトファイルに保存する必要がありました。
そして、{\em load} することでその内容を \gnuplot に取り込めば、
あなたが必要とする回数だけ異なる出力形式、異なる出力ファイルに対して
再描画をすることができました。

%%% The same limitation arose when you resized a multiplot displayed in 
%%% an interactive
...
同じ制限が、対話型出力形式で multiplot を resize した場合にも起こって
いました。それは、resize イベントは通常 replot を実行しているからです。
...
%%% \Gnuplot version 6 automates regeneration of all components in a multiplot.
%%% Starting when a command \textit{set multiplot} is seen, the program saves
%%% commands into a data block named \verb|$GPVAL_LAST_MULTIPLOT|.
%%% Once the closing command \textit{unset multiplot} is issued, this data 
%%% block
%%% can be replayed to redraw the corresponding set of plots that make up 
%%% the multiplot.
%%% This can be done explicitly using the \textit{remultiplot} command or 
%%% implicitly
%%% from \textit{replot} commands or resizing events if the immediately 
%%% preceeding plot
%%% command was part of a multiplot.  It is either a feature or a drawback 
%%% that the
%%% result of replaying the original commands may produce a slightly 
%%% different result
%%% if the state of the program (axis ranges, linetypes, fonts, etc) has 
%% changed in
%%% the interim.
\gnuplot バージョン 6 では、multiplot でのすべての要素の再生成を
自動化しました。コマンド \textit{set multiplot} があった場合、
\gnuplot はコマンド群を \verb|$PVAL_LAST_MULTIPLOT| %% $
という名前付きデータブロックに保存します。
終了するコマンド \textit{unset multiplot} を発行すると、
このデータブロックを、multiplot を作成する plot 命令群を再描画するもの
に置き換えます。コマンド \textit{remultiplot} を使用したときには、
それを明示的に行いますし、または直前の plot コマンドが multiplot の一部で
あるときに \textit{replot} コマンドかリサイズイベントを呼び出したときには、
暗黙にそれを行います。
...
%% end Japanese
%%% Revised for \gnuplot version 5 and 6 by Ethan Merritt.
\gnuplot バージョン 5 および 6 用の修正は、
